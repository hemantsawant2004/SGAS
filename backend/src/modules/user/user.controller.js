...
async function createUser(req, res) {
    const { username, given_name, role, class: studentClass, division, rollNumber, } = req.body;
    const desiredRole = role ?? "student";
    const creatorRole = req.user?.role;
    if (creatorRole === "admin" && desiredRole === "admin") {
        return res
...
async function signup(req, res) {
    const { username, password, role, class: studentClass, division, rollNumber, } = req.body;
    const desiredRole = role ?? "student";
    if (desiredRole === "admin") {
        return res.status(403).json({ message: "Admin signup is not allowed" });
    }
    ...
    await (0, user_service_1.updateUserPassword)(existing.id, hashed);
    return res.json({ message: "Signup completed" });
}
